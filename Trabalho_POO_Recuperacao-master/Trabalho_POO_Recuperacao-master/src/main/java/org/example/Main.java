...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            // Inserção
            String sqlInsert = "INSERT INTO produtos (id, nome, sobre, valor) VALUES (?, ?, ?, ?)";
            PreparedStatement pstmtInsert = conn.prepareStatement(sqlInsert);
            pstmtInsert.setInt(1, 1);
            pstmtInsert.setString(2, "Produto1");
            pstmtInsert.setString(3, "sobre o Produto1");
            pstmtInsert.setDouble(4, 10.50);
            pstmtInsert.executeUpdate();

            // Atualização
            String sqlUpdate = "UPDATE produtos SET valor = ? WHERE id = ?";
